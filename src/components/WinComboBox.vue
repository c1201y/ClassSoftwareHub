<template>
  <div
    ...
    <!-- 补丁（必填星号）：ClassSoftwareHub 提交页需要必填项红色星号；
         JS 组件的 header 插槽无法通过 vue-tsc 类型检查，故用 prop 方案 -->
    <div v-if="Header" class="win-combo-header">
      {{ Header }}<span v-if="RequiredMark" class="win-combo-required-mark" aria-hidden="true">*</span>
    </div>
...
/* 补丁（必填星号）：与 WinUI SystemFillColorCritical 一致，深浅色自适应 */
.win-combo-required-mark {
  margin-left: 4px;
