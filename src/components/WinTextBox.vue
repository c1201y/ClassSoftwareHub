...
/* 补丁（聚焦动画）：原实现是 display:none → block 硬切，蓝条“啪”地出现。
   改为常驻 + scaleX 从中心展开（WinUI 的 FocusBorder 动画），失焦时收回中心并淡出。
   搜索框 WinAutoSuggestBox、密码框等均复用本组件，故全站输入框一起生效。 */
...
/* 曲线说明：原先用的 cubic-bezier(.1,.9,.2,1) 前段太陡（70ms 就展开 96%），
   肉眼看几乎是瞬切。换成缓入缓出 + 300ms，展开过程才看得见。 */
...
/* 注意：这里刻意不写 @media (prefers-reduced-motion: reduce) 降级——
   按项目需求动画常驻开启，即使系统设置了“减少动态效果”也照常播放。 */
...
.win-textbox.is-focused:not(.is-disabled) .win-textbox-focus-border::after {
  transform: scaleX(1);
  height: 2px;
}
