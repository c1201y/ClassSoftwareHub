...
- 根目录 `文字设置.ts` 里 `app.version` 沿用了原版 HTML 的写法 `v2026831`
  （原版就这样，疑似手误写成 v2026831 而非 v20260831，如需修正改这一处即可）。
- 设置页“关于”里的“投喂作者/回声洞/作者首页”链接来自 `文字设置.ts` 的 `about.*-url` 键，改文字区即可换链接。
...
- **欢迎弹窗**：进入网站（打开后落在首页）会弹一个 WinUI 风格的欢迎窗（欢迎语/表情图/站点介绍/
  仓库地址 · 作者首页 · 相关文章 · 投喂作者 四个外链），点“开始探索下载~”关闭继续使用；
  带 `#/download/xxx`、`#/settings` 等直达地址打开不弹。
  弹窗文字 = `文字设置.ts` 的 `welcome.*` 键（相关文章链接 = `welcome.article-url`，
  投喂作者 = `about.reward` + `about.reward-url`）；表情图 = `src/assets/welcome-sticker.gif`
  （想换图就用同名文件覆盖后重新打包）。
