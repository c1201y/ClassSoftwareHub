...
## v2.3.1 - September 18 Incident（20260917PR01）发布！

本次更新新增「系统镜像下载」小工具与「内置工具」页搜索框，首页新增「加入 QQ 群」入口，把「内置工具」全面接入站点的 WinUI 界面组件（观感统一），并移除了首页那个早年用来应对服务器不稳的「下载 HTML」按钮；软件图标的加载也稳了，个别加载不出来的图标不再开天窗。
